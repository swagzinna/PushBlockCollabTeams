...
public class PushBlockEnvController : MonoBehaviour
{
    ...
    private SimpleMultiAgentGroup teamGreen;
    ...
    void Start()
    {
        ...
        if (m_PushBlockSettings.isMultiTeam)
        {
            ...
            teamGreen = new SimpleMultiAgentGroup();
        }
        else
        {
        ...
        foreach (var item in AgentsList)
        {
            ...
            if (m_PushBlockSettings.isMultiTeam)
            {
                if (item.Agent.CompareTag("redAgent"))
                {
                    teamRed.RegisterAgent(item.Agent);
                }
                else
                {
                    teamGreen.RegisterAgent(item.Agent);
                }
            }
            else
            {
    ...
    void FixedUpdate()
    {
        ...
        if (m_PushBlockSettings.isMultiTeam)
        {
            ...
            teamGreen.AddGroupReward(-0.5f / MaxEnvironmentSteps);
        }
        else
        {
    ...
    /// </summary>
    public void ScoredAGoal(Collider blockCol, Collider goalCol, float score)
    {
        ...
        //Disable the block
        blockCol.gameObject.SetActive(false);

        //Give Agent Rewards
        if (m_PushBlockSettings.isMultiTeam)
        {
            if (goalCol.CompareTag("redGoal"))
            {
                teamRed.AddGroupReward(score);
                teamGreen.AddGroupReward(-score);

                //Swap ground material for a bit to indicate we scored.
                StartCoroutine(GoalScoredSwapGroundMaterial(m_PushBlockSettings.teamRedGoalScored, 0.5f));
    ...
    public void ResetScene()
    {
        ...
        if(m_PushBlockSettings.calcAverageTime && timeCounter <= timedRounds)
        {
            if (timeCounter <= timedRounds)
            {
                ...
                avgTime = avgTime + ellapsed;
                timeCounter++;

                if(timeCounter > timedRounds)
                {
                    avgTime = avgTime / (timedRounds);
                    print("Avg: " + avgTime);
                }
